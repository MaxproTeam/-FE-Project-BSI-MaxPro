...
const workChecklistSPVPage = {
    getWorkChecklists : async () => {
        if(window.isWorkChecklistSPV) {
            try {
                ...
                const result = await module.getWorkOrders({status: 4});
...
const updateData = async (filters) => {
    try {
        ...
        const result = await module.getWorkOrders({filter, filterDate, status: 4});
...
export default workChecklistSPVPage;